...
Citizen.CreateThread(function()
  ...
  core.table.merge = function(t1, t2)
    t1 = t1 or {}
    if not t2 then return t1 end
    for k, v in pairs(t2 or {}) do
      if type(v) == "table" then
        if type(t1[k] or false) == "table" then
          core.table.merge(t1[k] or {}, t2[k] or {})
        else
          t1[k] = v
        end
      else
  ...
  core.table.mergeAfter = function(t1, t2)
    t1 = t1 or {}
    if not t2 then return t1 end
  ...
  core.table.filter = function(t, filterIter, keepKeyAssociation)
    local out = {}
  ...
  core.table.map = function(t, func)
    local new_table = {}
    for i, v in pairs(t or {}) do
      new_table[i] = func(v, i, t)
    end
    return core.table.copy(new_table)
  end
  ...
  core.table.find = function(t, func)
    for i, v in pairs(t or {}) do
      if func(v, i, t) then
        return v, i
      end
  ...
  core.table.clearForNui = function(t)
    local new_table = {}
    for key, data in pairs(t) do
      if type(data) == "function" then
      elseif type(data) == "table" then
        new_table[key] = core.table.clearForNui(data)
      else
        new_table[key] = data
      end
    end
  ...
  core.table.isEgal = function(table1, table2, strict, canMissInTable1, canMissInTable2)
    ...
    for key, value in pairs(table1) do
      if not canMissInTable2 or table2[key] ~= nil then
        if not core.table.isEgal(value, table2[key], strict, canMissInTable1, canMissInTable2) then
          return false
        end
      end
    end
    ...
    if strict then
      for key, value in pairs(table2) do
        if not canMissInTable1 or table1[key] ~= nil then
          if not core.table.isEgal(value, table1[key], strict, canMissInTable1, canMissInTable2) then
            return false
          end
        end
      end
  ...
  core.table.extract = function(t, key)
    if not t then return nil end
    local value = type(t[key]) == "table" and core.table.copy(t[key]) or t[key]
    t[key] = nil
    return value
  end
  ...
  core.table.addMultiLevels = function(...)
    local keys = { ... }
    local main = {}
  ...
  core.table.slice = function(t, s, e)
    s = s or 1
    e = e or s
    local result = {}
  ...
  core.table.doesKeyExist = function(t, ...)
    local keys = { ... }
  ...
  core.table.includes = function(t, value, fromIndex)
  ...
  core.table.deleteDeepValue = function(t, keys)
  ...
  core.table.deleteAndClear = function(t, keys)
    ...
    if core.table.isEmpty(current) then
      for i = parentsCount, 1, -1 do
        local parent = parents[i]
        local key = parents[i + keysLen]
        parent[key] = nil
        if not core.table.isEmpty(parent) or parent == t then break end
      end
    end
  ...
  core.table.getDeep = function(t, keys)
