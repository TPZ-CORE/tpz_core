...
core.component.data.pedCategories = {
  "heads",
  "eyes",
  "teeth",
  "bodies_upper",
...
core.component.data.horseCategories = {
  "horse_heads",
  "horse_bodies",
  "horse_feathers",
  "horse_blankets",
...
for i = 1, #core.component.data.pedClothes do
  local category = core.component.data.pedClothes[i]
  local hash = core.component.getCategoryHash(category)
  core.component.data.clothesCategories[hash] = category
end

core.component.data.categoryName = {}
for i = 1, #core.component.data.order do
  local category = core.component.data.order[i]
  local hash = core.component.getCategoryHash(category)
  core.component.data.categoryName[hash] = category
end

core.component.data.wearableStates = {
  shirts_full = {
...
core.component.wearableStates = core.component.data.wearableStates --deprecated name

core.component.data.palettes = {
  "metaped_tint_animal",
...
core.component.palettes = core.component.data.palettes --deprecated name
...
core.component.data.expressions = {
...
core.component.expressions = core.component.data.expressions --deprecated name
...
core.UpdatePedVariation = function(ped) 
...
core.IsPedReadyToRender = function(ped)
  repeat Wait(0) until Citizen.InvokeNative(0xA0BC8FAED8CFEB3C, ped or PlayerPedId())
end

core.EquipMetaPedOutfitPreset = function(ped, value) Citizen.InvokeNative(0x77FF8D35EEC6BBC4, ped, value, true) end
...
core.ApplyShopItemToPed = function(comp, ped)
...
core.GetNumComponentsInPed = function(ped) return invokeNative(0x90403E8107B60E81, ped) or 0 end
...
core.UpdateShopItemWearableState = function(comp, wearable)
...
core.GetCategoryOfComponentAtIndex = function(ped, componentIndex)
  local pedType = IsThisModelAHorse(GetEntityModel(ped)) and 6 or 0
  local category = invokeNative(0x9b90842304c938a7, ped, componentIndex, pedType, Citizen.ResultAsInteger())
  --patch neckerchiefs
  if category == `neckerchiefs` then
...
function Component.new(index, category, hash, wearableState, drawable, albedo, normal, material, palette, tint0, tint1, tint2)
  return {
    ...
    categoryHash = core.component.getCategoryHash(category),
    palette = palette,
    tint0 = tint0,
    tint1 = tint1,
    tint2 = tint2,
...
core.GetComponentAtIndex = function(ped, componentIndex, isMultiplayer)
...
core.IsMetaPedUsingComponent = function(categoryHash, ped)
  return Citizen.InvokeNative(0xFB4891BD7578CDC1, ped or PlayerPedId(), categoryHash)
end
...
function core.component.getBaseLayer(ped, hash, inTable)
  ...
  if inTable then
    return {
      ...
      normal = normal,
      material = material,
      palette = core.component.getPaletteNameFromHash(palette),
      tint0 = tint0,
      tint1 = tint1,
      tint2 = tint2
    }
  end
  return drawable, albedo, normal, material, core.component.getPaletteNameFromHash(palette), tint0, tint1, tint2
end
...
local function convertToMetaTag(ped, data)
  ...
  local drawable, albedo, normal, material, palette, tint0, tint1, tint2 = core.component.getBaseLayer(ped, data.hash)
...
local function applyDefaultBodyParts(ped)
  if IsPedMale(ped) then
    ...
    core.component.apply(ped, "bodies_upper", `CLOTHING_ITEM_M_BODIES_UPPER_001_V_001`)
    core.component.apply(ped, "bodies_lower", `CLOTHING_ITEM_M_BODIES_LOWER_001_V_001`)
    core.component.apply(ped, "heads", `CLOTHING_ITEM_M_HEAD_002_V_001`)
    core.component.apply(ped, "eyes", `CLOTHING_ITEM_M_EYES_001_TINT_001`)
    core.component.apply(ped, "teeth", `CLOTHING_ITEM_M_TEETH_000`)
  else
    EquipMetaPedOutfitPreset(ped, 7, false)
    core.component.apply(ped, "bodies_upper", `CLOTHING_ITEM_F_BODIES_UPPER_001_V_001`)
    core.component.apply(ped, "bodies_lower", `CLOTHING_ITEM_F_BODIES_LOWER_001_V_001`)
    core.component.apply(ped, "heads", `CLOTHING_ITEM_F_HEAD_002_V_001`)
    core.component.apply(ped, "eyes", `CLOTHING_ITEM_F_EYES_001_TINT_001`)
    core.component.apply(ped, "teeth", `CLOTHING_ITEM_F_TEETH_000`)
  end
end
...
local function resetCachedColor(ped, category)
  if not core.cache.component.color[ped] then return end
  core.cache.component.color[ped][category] = nil
  if category == `neckwear` then
    core.cache.component.color[ped][`neckerchiefs`] = nil
  end
end
...
local function initCachePedComponents(ped)
  ...
  if not core.cache.component.color[ped] then
    local numComponent = GetNumComponentsInPed(ped)
...
local function reapplyComponentStats(ped)
  for category, list in pairs(core.component.data.wearableStates) do
    if core.component.isCategoryAClothes(category) then
      local isEquiped, index = core.component.isCategoryEquiped(ped, category)
      if isEquiped then
...
local function reapplyComponentsColor(ped)
  for i = 1, #core.component.data.order do
    local category = core.component.getCategoryHash(core.component.data.order[i])
    if core.cache.component.color[ped][category] then
      local data = core.cache.component.color[ped][category]
...
local function reapplyCached(ped)
  if not core.cache.component.color[ped] then return end
  delays["refresh" .. ped] = core.timeout.delay("jo_libs:component:reapplyCachedColor" .. ped,
    function() core.component.waitPedLoaded(ped) end, function()
      refreshPed(ped)
      -- core.component.waitPedLoaded(ped)
      reapplyComponentStats(ped)
      reapplyComponentsColor(ped)
      core.cache.component.color[ped] = nil
      refreshPed(ped)
    end)
end
...
--- @param ped integer (The entity ID)
function core.component.refreshPed(ped)
...
--- @param ped integer (The entity ID)
function core.component.waitPedLoaded(ped)
  Wait(30)
  local isReady = core.waiter.exec(function() return IsPedReadyToRender(ped) end)
  if not isReady then
    if Config.Debug then eprint("This ped is not loaded:", ped) end
    return
  end
end
...
function core.component.apply(ped, category, _data)
  local data = core.component.formatComponentData(_data)
  ...
  if not data then
    return print("Wrong component data structure", ped, category, json.encode(_data))
  end
  ...
  if data.hash and not data.remove then
    categoryHash, isMp = core.component.getComponentCategory(ped, data.hash)
    if not categoryHash then
      return print("Wrong component hash:", categoryName, data.hash)
    end
  end
...
function core.component.remove(ped, category)
  return core.component.apply(ped, category, 0)
end

--- A function to remove all clothing components from a ped
---@param ped integer (The entity ID)
function core.component.removeAllClothes(ped)
  for i = 1, #core.component.data.pedClothes do
    local category = core.component.data.pedClothes[i]
    core.component.remove(ped, category)
  end
end
...
function core.component.applyComponents(ped, components)
  if not ped then return end
  if not DoesEntityExist(ped) then return end
  if not components then return end

  core.component.removeAllClothes(ped)

  for i = 1, #core.component.data.pedClothes do
    local category = core.component.data.pedClothes[i]
    if components[category] then
      core.component.apply(ped, category, components[category])
    end
  end
end
...
function core.component.applySkin(ped, skin)
  print("applySkin", ped, json.encode(skin))
  if not ped then return end
  if not DoesEntityExist(ped) then return print("core.component.applySkin() => Ped doesn't exist:", ped) end
  ...
  if skin.model then
    ...
    if GetEntityModel(ped) ~= modelHash then
      if (ped ~= PlayerPedId()) then
        ...
        core.utils.loadGameData(modelHash, true)
        print("model loaded", skin.model)
  ...
  print("fix issue on body")
  applyDefaultBodyParts(ped)

  core.component.refreshPed(ped)
  core.component.waitPedLoaded(ped)

  print("start apply default body components")

  local headHash = skin.headHash or core.component.getHeadFromSkinTone(ped, skin.headIndex, skin.skinTone)
  print("headHash", headHash)
  if headHash and headHash ~= 0 then
    core.component.apply(ped, "heads", headHash)
  end

  local bodies_upper = skin.bodyUpperHash or
      core.component.getBodiesUpperFromSkinTone(ped, skin.bodiesIndex, skin.skinTone)
  print("bodies_upper", bodies_upper)
  if bodies_upper and bodies_upper ~= 0 then
    core.component.apply(ped, "bodies_upper", bodies_upper)
  end

  local bodies_lower = skin.bodyLowerHash or
      core.component.getBodiesLowerFromSkinTone(ped, skin.bodiesIndex, skin.skinTone)
  print("bodies_lower", bodies_lower)
  if bodies_lower and bodies_lower ~= 0 then
    core.component.apply(ped, "bodies_lower", bodies_lower)
  end
  ...
  for expression, value in pairs(skin.expressions or {}) do
    ...
    SetCharExpression(ped, core.component.data.expressions[expression], percent)
  end
  ...
  local eyes = skin.eyesHash or core.component.getEyesFromIndex(ped, skin.eyesIndex)
  core.component.apply(ped, "eyes", eyes)

  local teeth = skin.teethHash or core.component.getTeethFromIndex(ped, skin.teethIndex)
  core.component.apply(ped, "teeth", teeth)

  core.component.apply(ped, "hair", skin.hair)
  if skin.model == "mp_male" then
    core.component.apply(ped, "beards_complete", skin.beards_complete)
  end
  ...
  --  core.pedTexture.overwriteBodyPart(ped, "heads", skin.overlays, true)
  --end

  Wait(100)

  SetPedScale(ped, skin.bodyScale)
  print("done create ped")
...
function core.component.setWearableState(ped, category, data, state)
  ...
  if index >= 0 then
    ...
    if wearableState == state then
      return false, print("State already equiped")
    end
  end
  ...
  if categoryHash == `neckwear` then
    if stateHash == `base` then
      local beardHard, beard = core.component.getComponentEquiped(ped, "beards_complete")
      if beardHard then
        core.component.apply(ped, beard.category, beard)
      end
    elseif stateHash == `mask_up` then
      --Dirty fix of beard
      local beardIndex = getComponentIndexOfCategory(ped, "beards_complete")
      if beardIndex >= 0 then
        local armorHash, armor = core.component.getComponentEquiped(ped, "armor")
        if armorHash then
          local shirtHash, shirt = core.component.getComponentEquiped(ped, "shirts_full")
          ...
          CreateThread(function()
            ...
            core.component.apply(ped, armor.category, armor)
          end)
        end
      end
    end
  ...
  if categoryHash == `boots` then
    local pantHash, pant = core.component.getComponentEquiped(ped, "pants")
    if pantHash then
      CreateThread(function()
...
function core.component.getWearableState(ped, category)
  ...
  local wearableStatesName = core.component.getWearableStateNameFromHash(wearableState)
...
function core.component.neckwearIsUp(ped)
  return core.component.getWearableState(ped, "neckwear") == "mask_up"
end

core.component.isNeckweaUp = core.component.neckwearIsUp

--- Return if the sleeve are rolled
---@param ped integer (The entity ID)
---@return boolean (Return `true` if the sleeve are rolled, `false` otherwise.)
function core.component.sleeveIsRolled(ped)
  return core.component.getWearableState(ped, "shirts_full"):find("rolled") ~= nil
end

core.component.isSleeveRolled = core.component.sleeveIsRolled

--- Return if the collar is opened
---@param ped integer (The entity ID)
---@return boolean (Return `true` if the collar is opened, `false` otherwise.)
function core.component.collarIsOpened(ped)
  return core.component.getWearableState(ped, "shirts_full"):find("open") and true or false
end

core.component.isCollarOpened = core.component.collarIsOpened
...
function core.component.sleeveUnroll(ped, hash)
  if core.component.isCollarOpened(ped) then
    ...
    core.component.setWearableState(ped, "shirts_full", hash, "base")
  end
end

core.component.unrollSleeve = core.component.sleeveUnroll
...
core.component.rollSleeve = core.component.sleeveRoll
...
core.component.openCollar = core.component.collarOpen
...
core.component.closeCollar = core.component.collarClose

--- A function to know if the boots are under the pant
---@param ped integer (The entity ID)
---@return boolean (Return `true` if boots are under the pant, `false` otherwise.)
function core.component.bootsAreUnderPant(ped)
  return core.component.getWearableState(ped, "boots") == "under_pants"
end

core.component.isBootsUnderPant = core.component.bootsAreUnderPant

--- A function to know if the vest is under the pant
---@param ped integer (The entity ID)
---@return boolean (Return `true` if the vest is in the pant, `false` otherwise)
function core.component.vestIsUnderPant(ped)
  return core.component.getWearableState(ped, "vests") == "under_pants"
end

core.component.isVestUnderPant = core.component.vestIsUnderPant
...
function core.component.loadoutIsOnRight(ped)
  return core.component.getWearableState(ped, "loadouts") == "base_right"
end

core.component.isLoadoutOnRight = core.component.loadoutIsOnRight

--- A function to know if the hair is pomaded
--- @param ped integer (The entity ID)
--- @return boolean (Return `true` if the hair is pomaded)
function core.component.hairIsPomade(ped)
  return core.component.getWearableState(ped, "hair") == "pomade"
end
...
function core.component.getCategoriesEquiped(ped)
  local categories = {}
  local numComponent = GetNumComponentsInPed(ped)
  for index = 0, numComponent - 1 do
    local category = GetCategoryOfComponentAtIndex(ped, index)
...
function core.component.isCategoryEquiped(ped, category)
  local index = getComponentIndexOfCategory(ped, category)
...
function core.component.getComponentEquiped(ped, category)
  local index = getComponentIndexOfCategory(ped, category)
  if index == -1 then
    return false, {}
  end
...
function core.component.getComponentsEquiped(ped)
  local components = {}
  local numComponent = GetNumComponentsInPed(ped)
  for index = 0, numComponent - 1 do
...
function core.component.getCategoryTint(ped, category, inTable)
...
function core.component.isCategoryAClothes(category)
  category = core.component.getCategoryHash(category)
  return core.component.data.clothesCategories[category] ~= nil
end
