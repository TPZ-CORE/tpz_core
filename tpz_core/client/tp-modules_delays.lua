...
---@param ... any (Additional arguments to pass to the callback function)
---@return TimeoutClass (Return the timeout instance)
function timeout.delay(id, msec, cb, ...)
  if delays[id] then
    delays[id]:clear()
    delays[id] = nil
  end
  delays[id] = timeout.set(msec, cb, ...)
  delays[id].id = id
  return delays[id]
end
...
---@param ... any (Additional arguments to pass to the callback function)
---@return TimeoutClass (Return the timeout instance)
function timeout.noSpam(id, msec, cb, ...)
  if delays[id] then
    delays[id]:clear()
    delays[id] = nil
    delays[id] = timeout.set(msec, cb, ...)
    ...
    delays[id] = timeout.set(msec, function() end)
    delays[id].id = id
    cb(...)
    return delays[id]
  end
