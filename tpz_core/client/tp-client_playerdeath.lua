...
local DeathData = { cooldown = 0, alertCooldown = 0, isDead = false, cameraHandler = nil, cameraAngleY = 0.0, cameraAngleZ = 0.0}
...
AddEventHandler('tpz_core:resurrectPlayer', function(cb)
  ...
  DeathData.alertCooldown = 0
  DeathData.isDead        = false
end)

RegisterNetEvent('tpz_core:applyLethalDamageToPlayerPed')
AddEventHandler('tpz_core:applyLethalDamageToPlayerPed', function()
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsEntityDead(player) then
      ...
      -- The following prompts such as alert and respawn will be visible only if the player is not being carried.
      if DeathData.isDead and not IsEntityAttachedToAnyPed(player) then
        ...
        for index, prompt in pairs(PromptsList) do
          ...
          if prompt.action == "ALERT" then
            ...
            if DeathData.alertCooldown <= 0 then
              PromptSetEnabled(prompt.prompt, 1)
            else
              PromptSetEnabled(prompt.prompt, 0)
            end
          ...
          if PromptHasHoldModeCompleted(prompt.prompt) then
            ...
            if prompt.action == "RESPAWN" then
              ...
              -- TP Realistic Flie Swamping (Removing PTFX).
              if Config.OnPlayerDeath.TPRealisticFlieSwamping then
                TriggerEvent("tp_realistic_flieswamping:stopFliesSwampingPtfx", true, true)
              end
            ...
            elseif prompt.action == "ALERT" then
              ...
              if Config.OnPlayerDeath.IncreaseRespawnIfAlertedBy then
                DeathData.cooldown = DeathData.cooldown + Config.OnPlayerDeath.IncreaseRespawnIfAlertedBy
              end
              
              TriggerEvent("tpz_core:ExecuteServerCallBack", "tpz_pigeon_alerts:hasPigeon", function(hasPigeon)

                if hasPigeon then
                    DeathData.alertCooldown = Config.OnPlayerDeath.PromptKeys['ALERT'].cooldown

                  TriggerEvent("tpz_pigeon_alerts:AlertDoctorsOnPlayersDeath")
                else
                  TriggerEvent('tpz_core:sendRightTipNotification', Locales['NOT_TRAINED_PIGEON'], 5000)
                end
...
-- Cooldown system when the player is dead or when alerting.
Citizen.CreateThread(function ()
  while true do
    Wait(1000)
    ...
    if DeathData.alertCooldown > 0 then
      ...
      DeathData.alertCooldown = DeathData.alertCooldown - 1
...
OnPlayerRespawn = function()
  ...
  --SetEntityHealth(player, Config.HealthOnRespawn)
